...
class PublisherSubscriber(Node):
	...
	def __init__(self):
		super().__init__('robotl_app')
		self.publisher_ = self.create_publisher(Twist, '/robot/cmd_vel', 10)
		self.subscription = self.create_subscription(LaserScan, '/robot/scan', self.callback, 10)
		self.msg = Twist()
...
def main(args=None):
    ...
    robotl_app = PublisherSubscriber()

    rclpy.spin(robotl_app)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    robotl_app.destroy_node()
    rclpy.shutdown()
